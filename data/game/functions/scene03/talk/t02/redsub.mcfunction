...
execute if score n RTalk matches 2 run tellraw @p[team=red] ["",{"text":"[もう一人の幽霊について聞きたい]","color":"dark_red","clickEvent":{"action":"run_command","value":"/execute if score *exec2 Grobal matches 0 if score click Any matches 2 if score state TalkState matches 4 run function game:scene03/talk/t01/redsub1"}},{"text":" "},{"text":"[この世界から出る方法を聞きたい]","color":"blue","clickEvent":{"action":"run_command","value":"/execute if score *exec2 Grobal matches 0 if score state TalkState matches 4 if score click Any matches 2 run function game:scene03/talk/t02/redsub2"}}]
