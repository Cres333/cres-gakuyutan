...
execute if score n Talk matches 2 run tellraw @p[team=blue] ["",{"text":"[もう一度話を聞きたい]","color":"dark_red","clickEvent":{"action":"run_command","value":"/execute if score hand Any matches 1 run function game:scene03/talk/t01re"}},{"text":" "},{"text":"[手に持っているアイテムを渡す]","color":"blue","clickEvent":{"action":"run_command","value":"/execute if score hand Any matches 1 run function game:scene03/talk/item/switcher"}}]

execute if score n Talk matches 1 run tellraw @p[team=red] "＃＃ＳＧ＃＃＃＃Ｐ＃"

execute unless score n Talk = max Talk run schedule function game:scene03/talk/t01sub 5s
