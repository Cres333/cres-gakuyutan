...
execute if score n Talk matches 1 run tellraw @p[team=blue] "詳しくはあまり話したことが無いから知らないです"
execute if score n Talk matches 1 run tellraw @p[team=blue] "この本を読んで、記憶が少しでも戻れば、"
execute if score n Talk matches 1 run tellraw @p[team=blue] "なにか覚えているかもしれませんが"
