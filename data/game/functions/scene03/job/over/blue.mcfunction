...
execute if score *t _OverBlue matches 60 store success score *clear _OverBlue run clear @p[team=blue] minecraft:blue_dye 1
execute if score *t _OverBlue matches 60 if score *clear _OverBlue matches 1 run tellraw @p[team=blue] "[!] ロウソクが1本無くなりました"
execute if score *t _OverBlue matches 60 if score *clear _OverBlue matches 0 run function game:scene03/job/over/set
